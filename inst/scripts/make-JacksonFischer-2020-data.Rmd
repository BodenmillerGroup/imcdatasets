...
The data is downloaded from zenodod.
The first zip folder contains the main single-cell, sample and patient metadata. The single-cell locations and cluster labels are downloaded as separate zip folders
because they were uploaded to zenodo separately at a later time point.
...
download.file(url.cells, destfile = file.path(workdir, "SingleCell_and_Metadata.zip"))

#Unzip file in same directory
system2("unzip", args = c("-o", # include override flag
                         file.path(workdir, "SingleCell_and_Metadata.zip"),"-d",workdir), stdout = TRUE)
...
download.file(url.cluster, destfile = file.path(workdir, "singlecell_cluster_labels.zip"))

#Unzip file in same directory
system2("unzip", args = c("-o", # include override flag
                         file.path(workdir, "singlecell_cluster_labels.zip"),"-d",workdir), stdout = TRUE)
...
download.file(url.clocations, destfile = file.path(workdir, "singlecell_locations.zip"))
...
##Read single-cell, sample and patient metadata of the Basel dataset

```{r read single-cell and metadata}
#Single cell expressions and spatial features
cells <- fread(file.path(workdir,"Data_publication/BaselTMA/SC_dat.csv"))

#Sample and patient metadata
meta <- fread(file.path(workdir,"Data_publication/BaselTMA/Basel_PatientMetadata.csv"))

#Panel info
panel <- fread(file.path(workdir,"Data_publication/Basel_Zuri_StainingPanel.csv"))

#Cluster labels (combine PhenoGraph cluster labels and metacluster labels)
PG_clusters <- fread(file.path(workdir,"Cluster_labels/PG_basel.csv"),header = T)
Meta_clusters <- fread(file.path(workdir,"Cluster_labels/Basel_metaclusters.csv"),header = T)
...
##Select a set of 100 patients as an example dataset (1 primary tumor core per patient)
...
spatial <- cells[channel %in% c("Area","Eccentricity","Solidity","Extent","EulerNumber", "Perimeter","MajorAxisLength","MinorAxisLength","Orientation", "Percent_Touching","Number_Neighbors" ),]

#Marker expression channels go into counts
cells <- cells[!channel %in% c("Area","Eccentricity","Solidity","Extent","EulerNumber", "Perimeter","MajorAxisLength","MinorAxisLength","Orientation", "Percent_Touching","Number_Neighbors" ),]
...
cells_raw <- dcast.data.table(cells,"core + CellId + id ~channel",value.var = "mc_counts")
rnames <- cells_raw$id
...
cells_quant <- dcast.data.table(cells,"core + CellId + id ~channel",value.var = "quant_norm")
...
cells_asinh <- dcast.data.table(cells,"core + CellId + id ~channel",value.var = "arcsinh")
...
#Make sce
sce <- SingleCellExperiment(assays = list(raw = cells_raw, quant_norm = cells_quant, arcsinh = cells_asinh))
cells_asinh <- NULL
cells_quant <- NULL
cells_raw <- NULL
cells <- NULL
...
singlecell_meta <- dcast.data.table(spatial,"core + CellId + id ~channel",value.var = "mc_counts")

#Add single-cell locations
singlecell_meta <- merge(singlecell_meta,locations[,c("Location_Center_X","Location_Center_Y","id")],by = "id")

#Add clusters
singlecell_meta <- merge(singlecell_meta,clusters[,c("PhenoGraphBasel","cluster","id")],by = "id")
setnames(singlecell_meta,"cluster","metacluster")

#Add sample and patient metadat
meta <- meta[,ImageNb := 1:.N]
singlecell_meta <- merge(singlecell_meta,meta[,-c("treatment","DiseaseStage","AllSamplesSVSp4.Array","ncores")],by = "core",all.x = T)

#Set ImageName,ImageNb and CellNb for cytomapper package to map to CytoImageList objects
...
rows <- merge(rows,unique(panel[,c("Metal Tag","FullStack")]),by = "Metal Tag",all.x = T)
...
download.file(url.imgs, destfile = file.path(workdir, "OMEandSingleCellMasks.zip"))

#Unzip main folder
system2("unzip", args = c("-o", # include override flag
                         file.path(workdir, "OMEandSingleCellMasks.zip"),"-d",workdir),stdout = TRUE)
...
system2("unzip", args = c("-o", # include override flag
                         file.path(workdir,"OMEnMasks", "Basel_Zuri_masks.zip"),"-d",workdir),stdout = TRUE)

#Unzip subfolder with tiffs
system2("unzip", args = c("-o", # include override flag
                         file.path(workdir,"OMEnMasks", "ome.zip"),"-d",workdir),stdout = TRUE)
...
Also, occasionally a single-cell ID is skipped in the masks but in the single-cell data the CellNbs
were renamed sequentially. Therefore, the single-cell IDs in the masks also have to renamed sequentially
in order to correspond to the CellNbs from the single-cell data.

```{r load and process masks}
#Load the masks as a CytoImageList object
masks <- loadImages(file.path(workdir,"Basel_Zuri_masks/"), pattern = "_full_maks.tiff")
...
#Ranem single-cell IDs sequentially in each mask
for (n in names(masks)){
  imageData(masks[[n]]) = mapvalues(imageData(masks[[n]]),sort(unique(as.integer(imageData(masks[[n]])))),0:(length(unique(as.integer(imageData(masks[[n]]))))-1))
}
...
file.remove(file.path(workdir,"ome/",infold[!infold %in% unique(sce$ImageName)]))
