...
# workdir <- tempdir()
workdir <- "/home/ubuntu/Data2/imcdatasets/"
...
system2("unzip", args = c(
    "-o", file.path(workdir, dataset_name),
...
mainExpName(sce_zurich) <- "JacksonFischer_2020_BreastCancer_Zurich"
...
mainExpName(sce_basel) <- "JacksonFischer_2020_BreastCancer_Basel"
...
```{r save-sce}
# Full dataset
mainExpName(sce) <- "JacksonFischer_2020_BreastCancer_FULL"
saveRDS(sce, file.path(outdir, "sce_full.rds"))
print(sce)

# Subampled Basel cohort
saveRDS(sce_basel, file.path(outdir, "sce.rds"))
print(sce_basel)
...
# Basel cohort
saveRDS(masks_basel, file.path(outdir, "masks.rds"))
...
```{r save-images-basel}
saveRDS(images_basel, file.path(outdir, "images.rds"))
...
saveRDS(images_zurich, file.path(outdir, "images.rds"))
