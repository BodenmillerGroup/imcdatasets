...
* The three data objects must contain matching information so that they can be associated by the [cytomapper](https://bioconductor.org/packages/cytomapper) package, as described in the [imcdatset vignette](https://www.bioconductor.org/packages/release/data/experiment/vignettes/imcdatasets/inst/doc/imcdatasets.html#5_Usage).
...
**For usability of the package and consistency across datasets, the data objects must be formatted as described in the guidelines described in the `Dataset format` section below.**
...
The `colData` entry of the `SingleCellExperiment` object is a `DataFrame` that contains observation metadata; i.e., cells, slides, tissue, patients, .... All column names must have a prefix that indicates the level of observation (e.g. `cell_`, `slide_` , `tissue_`, `patient_`, `tumor_`).

The following columns are required:

- `image_name`: unique image (ablated ROI) name. Should map to the `image_name` column in the metadata of the `images` and `masks` objects.
- `cell_number`: integer representing cell numbers. Should map to the values of cell segmentation masks.
- `cell_id`: a unique cell identifier defined as {`image_name` `_` `cell_number`} (e.g., `E02_188`).
...
* For post-translationally modified proteins:
  * Prefix the `name` with the modification type (e.g., `phospho-`, `methyl-`) and suffix it with the modified aminoacids (e.g., `[S28]`).
...
knitr::kable(
  marker_names,
  caption = "'name' and 'short_name' examples for some commonly used markers"
)
```

Finally, the `short_name` column should be used to name the rows of the `rowData` entry and as `rownames(sce)`.
...
It should at least contain a `counts` matrix with raw ion counts. The `assays` slot can also contain additional matrices with commonly used counts transformations, or counts transformations that were used in the article describing the dataset. For instance:  

* `exprs`: asinh-transformed counts, with a cofactor of 1.
* `quant_norm`: counts censored at the 99th percentile and scaled from 0 to 1.

Counts transformation should be documented in `man` pages (`.R` file containing the function call used to load the dataset).
...
Cell numbers in the `SelfHits` object are unique integers ranging from 1 to the number of cells in the dataset. These numbers should map to `cell_number_absolute` in `colData(sce)`.
