...
#' panels (Islet + Immune) that were acquired on two consecutive 4 µm sections. 
#' Each dataset (panel) is composed of three data objects: single cell data, 
#' multichannel images and cell segmentation masks.
#' The data was obtained by imaging mass cytometry (IMC) measuring 
#' formalin-fixed paraffin-embedded (FFPE) tissue
#' from 88 individuals along the disease progression of T1D. 
#' It contains, both Controls, single and multi-auto-antibody 
#' positive T1D donors (sAAb+/mAAb+), and donors with recent and 
#' long-standing T1D (Onset/LD).
#' 
...
#' @param panel which panel should be returned? Can be set to "islet" (default) 
#' or "immune".
#' @param full_dataset if FALSE (default), a subset of 120 informative images is returned. 
#' These are sampled along the disease progression of T1D. 
#' It also contains insulitic images, i.e. T-cell infiltration of islets. 
#' If TRUE, the full dataset (corresponding to 7024 images) is returned. 
...
#' All images can be retrieved from Zenodo.
...
#'
#' @details
#' This is an Imaging Mass Cytometry (IMC) dataset from Steenbuck, Damond et al. 
#' (2026):
#' \itemize{
#'     \item \code{images} contains 120 47-channel
...
#' \code{colData(sce)$cell_type} and immune cell clusters with 
#' \code{colData(sce)$cell_cluster_scaled}. The latter for the Immune panel only. 
#' 
...
#' objects.
#'
#' For more information, please refer to the Steenbuck, Damond, et al. publication.
#' 
...
#' 
#' File sizes:#' \itemize{
#'     \item \code{`images_immune`}: size in memory = 6.3 Gb, 
#'     size on disk = 650.3 Mb.
#'     \item \code{`masks_immune`}: size in memory = 138 Mb, 
#'     size on disk = 4.7 Mb.
#'     \item \code{`sce_immune`}: size in memory = 442 Mb, 
#'     size on disk = 217.1 Mb.
#'     \item \code{`masks_full_immune`}: size in memory = 8.6 Gb, 
#'     size on disk = 337.3 Mb.
#'     \item \code{`sce_full_immune`}: size in memory = 21.5 Gb, 
#'     size on disk = 13.0 Gb.
#'     \item \code{`images_islet`}: size in memory = 3.8 Gb, 
#'     size on disk = 738.8 Mb.
#'     \item \code{`masks_islet`}: size in memory = 83 Mb, 
#'     size on disk = 2.8 Mb.
#'     \item \code{`sce_islet`}: size in memory = 277 Mb, 
#'     size on disk = 128.5 Mb.
#'     \item \code{`masks_full_islet`}: size in memory = 5.1 Gb, 
#'     size on disk = 194.8 Mb.
#'     \item \code{`sce_full_islet`}: size in memory = 12.8 Gb, 
#'     size on disk = 6.8 Gb.
#' }
#'
...
#' in human pancreata. \emph{Nat Metab} https://doi.org/10.1038/s42255-026-01559-z
#' 
#' @examples
...
#'
#' @export
SteenbuckDamond_2026_T1D <- function (
    ...
    dataset_name <- "SteenbuckDamond_2026_T1D"
    ...
    if (!panel %in% c("islet", "immune"))
        stop('"panel" should be either "islet" or "immune".')
